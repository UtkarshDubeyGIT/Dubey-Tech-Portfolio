...
image: "/images/projects/laptop-predictor-1.png"
...
This project started with a simple question: **Can a machine learning model predict laptop prices from specifications?** Turns out, yes—and the journey from Jupyter notebook to deployed web app taught me more about ML engineering than months of theory.
...
- Robust to outliers (₹2L Apple laptops don't break the model)
...
MLmodels in notebooks are useless to non-technical users. Enter **Streamlit**—turn Python scripts into interactive web apps.

### App Architecture

```
app. py                    # Main Streamlit app
├── load_model()          # Unpickle trained pipeline
├── sidebar_inputs()      # User selection widgets
├── predict_price()       # Model inference
├── show_similar()        # Find comparable laptops
└── visualize_data()      # EDA charts
```
...
Users select specs → Model predicts price in real-time. 

#### 2. Similar Laptop Finder

```python
def find_similar(predicted_price, tolerance=5000):
    """Show real laptops within ₹5k of prediction"""
    similar = df[
...
Trained model on scikit-learn 1.2, deployed app with 1.3 → **Model failed to load.**
...
Trained with 25 brands.  User selects "Samsung" (not in training data) → **Error.**
...
**Translation**: Model explains 86% of price variance.  Average error: ₹12k (18%).
...
# Worst prediction:  ₹45k off
# Actual: ₹2,10,000 | Predicted: ₹1,65,000
...
3. **Streamlit makes ML accessible**: No need for React/Vue—turn notebooks into apps in 50 lines
4. **Deployment is 50% of the project**: Working model ≠ useful product
...
This project taught me that **ML isn't just about algorithms—it's about the entire pipeline**:
...
6. **Iteration**: Models aren't static—retrain, improve, repeat
...
*Built with ❤️ for making ML accessible to everyone*